...
const targetDate = new Date("2026-10-03T21:00:00").getTime();
...
function abrirAlias() {
  document.getElementById("aliasModal").classList.add("activo");
}

function cerrarAlias() {
  document.getElementById("aliasModal").classList.remove("activo");
}

function copiarAlias() {
  const texto = document.getElementById("textoAlias").innerText;
...
function copiarAlias() {
    ...
    const alias = "SOFIA.DANIELA.CERDAN";

    navigator.clipboard.writeText(alias);

    const aviso = document.getElementById("aliasCopiado");
